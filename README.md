...
[![CI](https://github.com/teddywinglee/social-media-insight-engine/actions/workflows/ci.yml/badge.svg)](https://github.com/teddywinglee/social-media-insight-engine/actions/workflows/ci.yml)
...
All LLM inference runs locally via [LM Studio](https://lmstudio.ai) — no API keys or cloud services required.

![Sample report](examples/report_sample.png)

## Prerequisites

- [LM Studio](https://lmstudio.ai) installed with a model loaded and the local server running (default: `http://localhost:1234`)
- [uv](https://docs.astral.sh/uv/) installed

Update `MODEL` in `pipeline/config.py` to match the model identifier shown in LM Studio.

## Setup

```bash
git clone https://github.com/teddywinglee/social-media-insight-engine
cd social-media-insight-engine
uv sync
```
...
├── pipeline/
│   ├── config.py        # LM Studio client and model name
